(function(angular) {
    ...
    angular.module('FileManagerApp').service('apiMiddleware', ['$window', 'fileManagerConfig', 'apiHandler', 
        ...
        ApiMiddleware.prototype.upload = function(files, path, extraData) {
            if (! $window.FormData) {
            ...
            return this.apiHandler.upload(fileManagerConfig.uploadUrl, destination, files, extraData);
        };
        ...
        ApiMiddleware.prototype.getImagePreview = function(item) {
            var itemPath = this.getFilePath(item);
            return this.apiHandler.getImagePreview(fileManagerConfig.downloadFileUrl, itemPath);
        };
